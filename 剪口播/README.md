...
- **BytePlus（預設）**：在 `scripts/.env` 填 `BYTEPLUS_API_KEY=你的key`。
- **本機 Whisper（免 key）**：裝好 [whisper](https://github.com/openai/whisper) 後用 `scripts/whisper_transcribe.sh`，全程離線。
...
- `用户习惯/`：規則檔，例如哪些連接詞永遠不准刪。直接改 md 檔就生效。
- `training_config.json`：所有可調參數（靜音閾值、偵測開關）。

## 更多文件

- `design.md`：pipeline 架構與每個設計的原因
- `testing.md`：改碼前該跑的測試
- `decisions.md`：架構決策記錄
...
MIT。問題回報開 issue，或來 [domyweb.org/tools/auto-edit](https://domyweb.org/tools/auto-edit/) 找我。
