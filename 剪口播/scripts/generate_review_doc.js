...
function render(){doc.innerHTML='';wordEl=[];words.forEach(function(w,i){var el=document.createElement('span');el.className=cls(i);el.dataset.idx=i;if(w.isGap){var d=w.end-w.start;el.textContent=d>=0.5?(' \\u233f'+d.toFixed(1)+'s '):' ';}else{el.textContent=w.text;var tip=autoReasons[i]||'';if(w._suspect)tip=(tip?tip+' | ':'')+'\\u26a0 \\u7591\\u4f3c\\u807d\\u932f\\uff0c\\u8b1b\\u7a3f\\u662f\\u300c'+(w._refHint||'?')+'\\u300d';if(tip)el.title=tip;}doc.appendChild(el);wordEl[i]=el;});updateStats();}
