...
<style>
  ...
  #doc{max-width:640px;margin:0 auto;background:#fff;border:1px solid #e3e1d9;border-radius:8px;padding:44px 52px;font-size:17px;line-height:2.2;min-height:300px;}
...
function render(){doc.innerHTML='';wordEl=[];var i=0;while(i<words.length){if(words[i].isGap){var rd=0;while(i<words.length&&words[i].isGap){rd+=words[i].end-words[i].start;wordEl[i]=null;i++;}doc.appendChild(rd>=1.2?document.createElement('br'):document.createTextNode(' '));continue;}var w=words[i],el=document.createElement('span');el.className=cls(i);el.dataset.idx=i;el.textContent=w.text;var tip=autoReasons[i]||'';if(w._suspect)tip=(tip?tip+' | ':'')+'\\u26a0 \\u7591\\u4f3c\\u807d\\u932f\\uff0c\\u8b1b\\u7a3f\\u662f\\u300c'+(w._refHint||'?')+'\\u300d';if(tip)el.title=tip;doc.appendChild(el);wordEl[i]=el;i++;}updateStats();}
