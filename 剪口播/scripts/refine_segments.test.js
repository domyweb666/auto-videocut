...
// ── 測試：Step C 刀口原子化以 index 為準（修「刪的字被 cut_snap 留回影片」）──
console.log('\n[測試] --delete-indices：刪的字整段補刀、留的字整段退刀');
{
  // 甲乙[長]期反：長(idx2) 被 index 刪；但內容刪除段只蓋到長的 0.05s 尾＋吃進留字期 0.2s
  const words = [
    { text: '甲', start: 0.0, end: 0.5, isGap: false }, // 0 留
    { text: '乙', start: 0.5, end: 1.0, isGap: false }, // 1 留
    { text: '長', start: 1.0, end: 1.2, isGap: false }, // 2 刪（0.2s）
    { text: '期', start: 1.2, end: 1.5, isGap: false }, // 3 留
    { text: '反', start: 1.5, end: 1.8, isGap: false }, // 4 留
  ];
  const del = [{ start: 1.15, end: 1.4 }]; // 只蓋長 0.05s 尾 + 期前 0.2s
  const cfg = { pause_flatten: { enabled: false }, cut_snap: { enabled: false }, word_atomic: { enabled: true, sliver_sec: 0.09 } };
  fs.writeFileSync(p('w2.json'), JSON.stringify(words));
  fs.writeFileSync(p('d2.json'), JSON.stringify(del));
  fs.writeFileSync(p('c2.json'), JSON.stringify(cfg));
  fs.writeFileSync(p('idx2.json'), JSON.stringify([2])); // 只有「長」被 index 刪
  const runC = (withIdx) => {
    const args = [path.join(__dirname, 'refine_segments.js'), p('w2.json'), p('d2.json'), p('nope.json'), p('nope.json'), p('o2.json'), p('c2.json')];
    if (withIdx) args.push('--delete-indices', p('idx2.json'));
    execFileSync('node', args, { stdio: 'pipe' });
    return JSON.parse(fs.readFileSync(p('o2.json'), 'utf8'));
  };
  const covers = (segs, a, b) => segs.some(s => s.start <= a + 0.011 && s.end >= b - 0.011);
  const overlaps = (segs, a, b) => segs.some(s => s.end > a + 0.011 && s.start < b - 0.011);

  const withoutIdx = runC(false);
  ok(!covers(withoutIdx, 1.0, 1.2), '無 index：覆蓋率判斷把「長」留在影片（重現 bug）');

  const withIdx = runC(true);
  ok(covers(withIdx, 1.0, 1.2), '有 index：「長」發音區被整段補刀刪除（影片＝審核頁）');
  ok(!overlaps(withIdx, 1.2, 1.5), '有 index：留字「期」被退刀保住（不誤刪）');
}
