...
- **點題→展開豁免（最常見的誤刪，優先檢查）**：A 是簡短的點題/總起/斷言（「那 X 就是長這樣」「習慣靠環境養成」），B 緊接著展開細節——這是教學修辭的遞進，不是重錄，→ keep_both。判別測試：重錄是「取代」（A 講壞或講短所以重來，成品只該留一次）；點題是「鋪墊」（A 完成了點題任務，B 不是重講 A 而是往下展開；刪掉 A 會讓 B 顯得沒頭沒尾）。
...
- 前「慢跑是有氧運動」/ 後「慢跑能練心肺耐力」→ keep_both（不同面向）
- 前「預算怎麼抓」/ 後「預算怎麼抓呢就是先分固定和變動」→ delete_earlier（後段是完整版）
- 前「那番茄鐘就是長這樣。」/ 後「番茄鐘就是先專注二十五分鐘，然後每輪之間……」→ keep_both（點題→展開：前句是總起，後句往下講細節，刪掉前句後句會沒頭）
- 前「習慣靠環境養成。」/ 後「好習慣是靠環境養成的，你的環境是怎麼安排的？」→ keep_both（斷言→提問展開，教學遞進不是重錄）
